...
> **Nomes:**  Fernando José dos Reis Cruz  **Turma:** ADS **Data:** _20__ / __08_ / ___2026___
...
Uso do POST, para consultar os dados deve se utilizar o GET
O recurso deve ser utilizado no substantivo.
...
1. Qual(is) problema(s) de design vocês identificam?
o GET está sendo utilizado para deletar e o retorno do status code 200 OK.
2. Seu redesenho (método + rota + status codes):
DELETE /api/alunos/7
...
1. Qual(is) problema(s) de design vocês identificam?
O servidor retorna 200 OK, mas o correto para criação de recurso é 201 CREATED
2. Seu redesenho (método + rota + status codes):
POST / api / aunos / 201 CREATED
location
...
1. Qual(is) problema(s) de design vocês identificam?
URL muito extensa e precisa de muitos recursos para acessar a disciplina.
2. Seu redesenho (método + rota + status codes):
...
1. Qual(is) problema(s) de design vocês identificam?
Essa api deveria retornar em JSON.
O HTTP não recebe o resultado
2. Seu redesenho (método + rota + status codes):
GET /api/alunos / 7 /matriculas 
Caso o aluno não exista: 404 Not Found
Content-Type: application/json, se o aluno existir: " 200 OK".
...
1. Qual(is) problema(s) de design vocês identificam?
O design mostra uma ação "atualizarNotaParcial" deveria ser algum recurso.
...
Eu adicionaria versões da API nas rotas, quando for criar uma nova versão utilizar os números inteiros sem quebrar a última atualização. exemplo version1, version2 e etc...
